...
int main() {
	...
	do { //gameloop
		...
		do {
			...
			std::cout << "You currently have " << player.p_current_health << " hit points!" << std::endl;
...
void exp_gain(Enemy monster, Hero &player) {
	...
	int exp_needed = player.p_level * 10;
...
int battle(bool &player_is_fast, Hero &player, Enemy &monster, bool player_attacks) {
	...
	if (player_attacks == true){
		if (player_is_fast == true) { // player hits first
			...
			std::cout << "It has " << monster.m_current_health << " hit points remaining.\n" << std::endl;
			...
			std::cout << "You have " << player.p_current_health << " hit points remaining!\n" << std::endl;
			turn_one_ended = true;
